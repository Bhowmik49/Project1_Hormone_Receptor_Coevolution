#!/bin/bash
# ----------------------------------------------
# RNA-seq Trimming Script for Anolis Dataset (ASC)
...
RAWDIR="${WD}/RNA_seq_data"
CLEANDIR="${WD}/cleaned_data/RNAseq_trimmed"
QCPOST="${WD}/fastqc_reports/PostCleanQC_RNAseq"
...
cd "$RAWDIR" || { echo " RNA_seq_data folder not found!"; exit 1; }
ls *_1.fastq | sed 's/_1.fastq//' > sample_list.txt

# Main loop
while read -r SAMPLE; do
    echo " Trimming RNA-seq sample: ${SAMPLE}"
...
echo " RNA-seq trimming complete for all samples."
