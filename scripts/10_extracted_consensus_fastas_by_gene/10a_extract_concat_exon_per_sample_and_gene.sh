...
#------------------------------------------------------------------
# Script: 10a_extract_concat_exon_per_sample_and_gene.sh
# Purpose: 
#   1. Extract exon regions from consensus FASTA files
...
OUT_SAMPLE_DIR="$ROOT/variants/exon_fasta_5/per_sample_per_gene_${FEATURE}_truthful_fastas_5"

# Output: multi-FASTA per gene across all samples
OUT_GENE_CONCAT="$ROOT/variants/exon_fasta_5a/per_gene_${FEATURE}_truthful_fastas_5_${FEATURE}"
...
for BED in "$BED_DIR"/*_"$FEATURE".bed.sorted; do
    ...
    for CONS in "$CONS_DIR"/*.fasta; do
        ...
        # Write to per-sample FASTA
        echo ">${SAMPLE}" > "$SAMPLE_FASTA"
        echo "$CONCAT_SEQ" >> "$SAMPLE_FASTA"

        # Also append to per-gene multi-FASTA
...
echo "[DONE] Per-sample and per-gene exon FASTAs created successfully."
